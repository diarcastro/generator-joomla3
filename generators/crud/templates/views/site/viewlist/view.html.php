...
/**
 * View class for a list of <%=s.capitalize(data.modelListName)%>.
 *
 */
class <%=data.names.list.view%> extends JViewLegacy{
	...
	*/
	...
	protected function addToolbar(){
		//require_once JPATH_COMPONENT . '/helpers/<%=data.projectName%>.php';
		...
		if (count($user->getAuthorisedCategories('<%=data.componentName%>', 'core.create')) > 0){
	...
	protected function getSortFields(){
		return [
			<% data.fields.forEach(function(field){ %>'<%=field.name%>'=>JText::_('COM_<%=data.projectName.uCase()%>_HEADING_<%=field.name.uCase()%>'),
			<% })%>
		];
	}
}
